...
{
    $configured = trim((string)(getenv('APP_URL') ?: getenv('APP_BASE_URL') ?: ''));
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        if ($user) {
            ...
            if (!filter_var(getenv('MAIL_ENABLED') ?: '0', FILTER_VALIDATE_BOOLEAN)) {
                $demoResetLink = $link;
            }
        ...
        $message = 'If an active account matches that email, a one-hour reset link is ready. Check your email or use the demo link shown below.';
