...
grep -q 'redirect_role_home' register.php || fail "Registration entry redirect must follow role home mapping"
grep -q "require_role(\\['user'\\])" dashboard.php complaints.php add_complaint.php || fail "Complainant modules must enforce user role guard"
grep -q 'role_complaints_page' view_complaint.php || fail "Complaint detail breadcrumb must use role-aware complaints path"
grep -q '\$is_owner' view_complaint.php || fail "Complaint detail actions must be owner-aware"
grep -q 'notify_new_comment' api/add_comment.php || fail "API comment flow must use shared notification/timeline workflow"
