...
// Ensure the presentation environment has a small sample queue even when a
// persistent host skips startup migrations. This is idempotent and admin-only.
$demoCountResult = $conn->query("SELECT COUNT(*) AS cnt FROM complaints WHERE admin_remarks = 'Demo case seeded for presentation'");
$demoCount = (int) ($demoCountResult?->fetch_assoc()['cnt'] ?? 0);
if ($demoCount < 4) {
